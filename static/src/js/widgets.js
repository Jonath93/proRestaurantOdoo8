        ...
        renderElement: function() {
            ...
            _.each(this.pos.cashregisters,function(cashregister) {
                var button = new module.PaypadButtonWidget(self,{
                 ...
                 var buttondelete = new module.DeleteButtonWidget(self,{
                    pos:self.pos,
                    pos_widget:self.pos_widget,
                    cashregister : cashregister,
                });
                button.appendTo(self.$el);
                buttondelete.appendTo(self.$el);
            });
        }
    });

//linea de pago del boton
    ...
    //boton para eliminar las ordenes 1.1
   /* module.DeleteButtonWidget = module.PosBaseWidget.extend({
      template: 'DeleteButtonWidget',
      init: function(parent, options){
        this._super(parent);
        this.state = new module.NumpadState();
         window.numpadstate = this.state;
      ...
      start: function(){
        this.state.bind('change:mode', this.changedMode, this);
        this.changedMode();
        this.$el.click( _.bind(this.clickdelete,this));
      },
